...
export default function PainManagement() {
  // ---------------------------------------------------------
  // ★ デザイン統一用リモコン（青ベース・全面透過ガラス＆背景斜め直線光源版）
  // ---------------------------------------------------------
  ...
  return (
    ...
    <section id="pain" className={`${sectionPadding} relative bg-[#0B111E] text-slate-400 overflow-hidden`}>
      ...
      {/* 💡 【修正】スクショの指示通り、線の光をカードの上ではなく「背景（z-0）」に完全移動。
          セクション全体を斜めに大きく切り裂く強力なシアンブルーの直線光源ラインです。
          エラー原因となる複雑な記述を全廃し、超安全なTailwindグラデーションのみでモヤと芯を表現しました。 */}
      <div className="absolute inset-0 z-0 pointer-events-none opacity-70">
        {/* 中心を走る鋭い斜めの光の線 */}
        <div className="absolute top-[-50%] left-[-50%] w-[200%] h-[200%] bg-[linear-gradient(135deg,transparent_49.5%,#38bdf8_50%,transparent_50.5%)] blur-[2px]" />
        {/* 周囲に大きく広がる強力な光のモヤ */}
        <div className="absolute top-[-50%] left-[-50%] w-[200%] h-[200%] bg-[linear-gradient(135deg,transparent_40%,#0ea5e9_50%,transparent_60%)] blur-[140px] opacity-60" />
      </div>
        ...
        <div className="space-y-12 sm:space-y-24 mb-20">
          {PAIN_CONTENT.measures.map((m, i) => (
            <div key={i} className="relative grid lg:grid-cols-12 gap-0 items-center" style={gpuStyle}>
              ...
              {/* 💡 【修正】画像フィールド：カードより前面（上）に被せるため「z-20」を強制指定 */}
              ...
              {/* 💡 【修正】テキストパネル：画像の下に潜り込ませるため「z-10」に設定。
                  マシーンと同じ、全面美しい磨りガラスの透過デザイン（bg-slate-900/40 backdrop-blur-2xl）です。
                  背景にある強力な斜めの直線光が、カードの裏側から綺麗に透き通って見えます。 */}
              <div className={`
                lg:col-span-6 
                relative z-10 
                mt-[-40px] lg:mt-0 
              ...
              `}>
                ...
                {/* カード内部の柔らかなアンビエント発光 */}
                <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(56,189,248,0.04),transparent_75%)] pointer-events-none z-0" />
