...
export default function PainManagement() {
  ...
  return (
    ...
    <section id="pain" className={`${sectionPadding} relative bg-[#0B111E] text-slate-400 overflow-hidden`}>
      ...
      {/* 美しい上下並びのバランスを最大化するため、横幅をmax-w-4xlに最適化 */}
          ...
          {PAIN_CONTENT.measures.map((m, i) => {
            ...
            return (
              <div key={i} className="relative flex flex-col items-center w-full gap-0">
                ...
                {/* 💡 【背景光源：ジグザグ仕様】
                    格子状を完全に排除し、パートごとに「右上から左下」「左上から右下」へと光の走る向きを交互（ジグザグ）にスイッチ。
                    マシーンパートと同じ、芯のあるシャープな複数ライン構成で、ガラスが最も美しく引き立つよう光強度を最大化。 */}
                <div className="absolute inset-0 z-0 pointer-events-none opacity-90 scale-110">
                  {isEven ? (
                    /* 偶数番目：右上から左下へ平行に突き抜ける強烈な2本線 */
                    <>
                      <div className="absolute inset-0 bg-[linear-gradient(135deg,transparent_46%,#38bdf8_46.3%,#38bdf8_46.6%,transparent_46.9%,transparent_52.1%,#0ea5e9_52.4%,#0ea5e9_52.7%,transparent_53%)] blur-[1px]" />
                      <div className="absolute inset-0 bg-[linear-gradient(135deg,transparent_40%,rgba(56,189,248,0.4)_48%,transparent_56%)] blur-[60px]" />
                    </>
                  ) : (
                    /* 奇数番目：左上から右下へ平行に突き抜ける強烈な2本線 */
                    <>
                      <div className="absolute inset-0 bg-[linear-gradient(45deg,transparent_46%,#38bdf8_46.3%,#38bdf8_46.6%,transparent_46.9%,transparent_52.1%,#0ea5e9_52.4%,#0ea5e9_52.7%,transparent_53%)] blur-[1px]" />
                      <div className="absolute inset-0 bg-[linear-gradient(45deg,transparent_40%,rgba(56,189,248,0.4)_48%,transparent_56%)] blur-[60px]" />
                    </>
                  )}
                  {/* 全体を底上げするグロウアンビエント */}
                  <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(14,165,233,0.15),transparent_70%)] blur-[40px]" />
                </div>

                {/* 背景の巨大ID番号 */}
                <span className="font-sans text-[10rem] sm:text-[16rem] font-black text-slate-950 opacity-[0.35] absolute -top-20 right-4 italic pointer-events-none select-none z-0">
                  {m.id}
                </span>

                {/* 1️⃣ 上側：画像フィールド（不透明度100%のクリーン表示、下カードに隙間なく密着） */}
                ...
                {/* 2️⃣ 下側：テキストパネル（💡 マシーンパートと完全に同期した100%リアルガラス仕様） */}
                <div className={`
                ...
                `}>
                  ...
                  {/* 💡 【工夫：画像とガラスカードの境界シームレス化】
                      画像は100%明るいまま、ガラスカードの最上部内側に、画像背景の黒（bg-slate-950）から
                      磨りガラスへと滑らかに溶け込ませるグラデーションレイヤーを配置。
                      さらに、境界線に極細のグロウハイライトを走らせることで、境目を美しく同化させます。 */}
                  <div className="absolute top-0 inset-x-0 h-16 bg-gradient-to-b from-slate-950 via-slate-900/20 to-transparent opacity-90 pointer-events-none z-10" />
                  <div className="absolute top-0 inset-x-0 h-[1px] bg-gradient-to-r from-transparent via-sky-400/40 to-transparent pointer-events-none z-10" />

                  {/* 💡 マシーンパートと全く同じ質感・高級感を放つ「2箇所のガラス表面反射」 */}
