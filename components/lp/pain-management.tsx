...
export default function PainManagement() {
  // ---------------------------------------------------------
  // ★ デザイン統一用リモコン（上下セパレート透過×背景斜め直線光源・安全版）
  // ---------------------------------------------------------
  ...
  return (
    ...
    <section id="pain" className={`${sectionPadding} relative bg-[#0B111E] text-slate-400 overflow-hidden`}>
      ...
      {/* 💡 【修正】強力な斜めの線の光を「背景（z-0）」に配置。
          セクション全体を斜めに大きく貫き、周囲にシアンブルーの光のモヤが広がる仕様です。
          ビルドエラーを確実に防ぐため、標準的なSVGフィルターでボカシを完全制御しています。 */}
      <div className="absolute inset-0 z-0 pointer-events-none">
        <svg className="w-full h-full" viewBox="0 0 1440 1200" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
          <defs>
            {/* 中心線のシャープなぼかし */}
            <filter id="pain-blur-core">
              <feGaussianBlur stdDeviation="3" />
            </filter>
            {/* 周囲に広く拡散する強力な光のモヤのぼかし */}
            <filter id="pain-blur-ambient">
              <feGaussianBlur stdDeviation="90" />
            </filter>
            {/* 斜め線用のグラデーション */}
            <linearGradient id="pain-diagonal-glow" x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          {/* ① 周囲に大きく広がる、強力な光のモヤ */}
          <line x1="-100" y1="100" x2="1600" y2="1100" stroke="#0ea5e9" strokeWidth="200" opacity="0.55" filter="url(#pain-blur-ambient)" />
          
          {/* ② 中心を鋭く走る斜めの光の線 */}
          <line x1="-100" y1="100" x2="1600" y2="1100" stroke="url(#pain-diagonal-glow)" strokeWidth="8" filter="url(#pain-blur-core)" />
        </svg>
      </div>
        ...
        <div className="space-y-12 sm:space-y-24 mb-20">
          {PAIN_CONTENT.measures.map((m, i) => (
            <div key={i} className="relative grid lg:grid-cols-12 gap-0 items-center" style={gpuStyle}>
              ...
              {/* 💡 【修正】画像フィールド：カードより前面（上）に被せるため「z-20」を指定 */}
              ...
              {/* 💡 【修正】テキストパネル：画像の下に潜り込ませるため「z-10」を指定 */}
              <div className={`
                lg:col-span-6 
                relative z-10 
                mt-[-40px] lg:mt-0 
              ...
              `}>
                ...
                {/* 🔴 ① タイトルより上のエリア：【透過なし】ソリッド背景
                    画像と被る最上部を完全な不透明（bg-[#111827]）にし、文字の可読性を100%完全に守ります。 */}
                <div className="p-8 sm:p-14 pb-6 bg-[#111827] relative z-10 space-y-4">
                ...
                {/* 🔴 ② タイトルより下のエリア：マシーンと同じ【透過ガラス仕様】
                    説明文が乗る下半分のみを、美しい磨りガラス透過（bg-slate-900/40 backdrop-blur-2xl）に。
                    背景を走る「強力な斜めの光の線」が、カードの裏側から滑らかに美しく透けて見えます。 */}
        ...
        <div className="max-w-4xl mx-auto pt-12 sm:pt-16 border-t border-slate-900/60">
          <div className="flex flex-col items-center text-center">
            ...
            <div className="space-y-8">
              ...
              {/* 💡 【バグ修正箇所】エラーの原因だった末尾の不要な記ホウを完全に削除し、クリーンに閉じました */}
