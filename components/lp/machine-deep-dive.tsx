...
export default function MachineDeepDive() {
  ...
  // 💡 【統一ベース】痛みパートに準拠した角丸と、すっきりした枠線
  const cardRounded = "rounded-xl"
  
  // 💡 【新デザイン：フロストグラス（磨りガラス）カード】
  // カードの背景を少し透過（/85）させ、backdrop-blur（ぼかし）を入れることで、
  // 背景に配置した「縦の光の線」が、カードの裏側から上品に透けて見えるようにします。
  const baseCardStyle = "border border-slate-700/50 bg-[#151F32]/85 backdrop-blur-md shadow-2xl"
  ...
  return (
    <section id="machine" className={`${sectionPadding} relative bg-[#0B111E] text-slate-400 overflow-hidden`}>
      ...
      {/* 💡 【パートの違いを生む工夫】背景を貫く「縦の一本の光（レーザーコア）」
          痛みパートにはない、このセクションだけの象徴的な光です。 */}
      <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-sky-400/60 to-transparent shadow-[0_0_20px_rgba(56,189,248,0.5)] z-0" />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-2 w-full h-full">
              ...
              {/* 左側：画像エリア（💡 光も幕も一切なし。100%クリアに表示） */}
              <div className="relative aspect-[12/9] lg:aspect-auto bg-[#0B111E] w-full overflow-hidden border-b lg:border-b-0 lg:border-r border-slate-700/50">
          ...
          <div className="mb-12 grid grid-cols-3 gap-3 sm:gap-6">
            {MACHINE_CONTENT.features.map((f, i) => (
              <motion.div 
                key={i} 
                className={`aspect-[4/3] relative overflow-hidden ${cardRounded} border border-slate-700/50 shadow-2xl bg-[#0B111E]`} 
          ...
          <motion.div className={`overflow-hidden ${cardRounded} ${baseCardStyle}`}>
            {MACHINE_CONTENT.reasons.map((reason, i) => (
              <div key={i} className={`relative grid md:grid-cols-2 items-center ${i !== 0 ? 'border-t border-slate-700/50' : ''}`}>
                ...
                {/* 画像側（💡 幕なし、エフェクトなし、100%クリア） */}
                <div className={`relative aspect-video bg-[#0B111E] overflow-hidden ${i % 2 !== 0 ? 'md:order-last' : ''} border-r border-slate-700/50`}>
                ...
                <div className={`${cardTextPadding} relative h-full flex flex-col justify-center`}>
                  ...
                  {/* 💡 カードの内部には光源を一切置かない。
                      代わりに、テキストの横に「背景の光の線」とリンクするような
                      短い縦のアクセントラインを置いて、洗練された統一感を出します。 */}
                  <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[2px] h-[40%] bg-sky-500/30 rounded-r-full" />
