...
import { Minus, ShieldCheck, Zap } from 'lucide-react'
import { motion } from 'framer-motion'
import { PAIN_CONTENT } from '@/edit/pain-content'

export default function PainManagement() {
  // ---------------------------------------------------------
  // ★ デザイン統一用リモコン（AGA・ジグザグ光＆透過カード決定版）
  // ---------------------------------------------------------
  ...
  const cardBorder = "border border-slate-900/60 border-t-white/10 border-l-white/5"
  
  /* 💡 【重要】透過カードの設定：
     カード本体の色を透過グレー（bg-[#0B0F17]/70）にし、透过させたことで、背後のジグザグ光が透けて見えるようにしました。透過グレーにすることで、高級なガジェットのような洗練された印象になります */
  const cardShadow = "shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5),0_30px_60px_-15px_rgba(56,189,248,0.1)] bg-[#0B0F17]/70"
  // ---------------------------------------------------------
  ...
  return (
    ...
    <section id="pain" className={`${sectionPadding} relative bg-[#0B111E] text-slate-400 overflow-hidden`}>
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        {/* 🔴 Measures (各施策) */}
        <div className="space-y-12 sm:space-y-24 mb-20 relative">
          
          {/* 💡 【新発明：ジグザグ線の光源】セクション全体を、パートごとにジグザグに走るスカイブルーの線の光源（光の帯）。
             単純な線ではなく、透過したスカイブルー（rgba(56,189,248,0.1)）の帯を斜めに重ねることで、洗練された光の演出を表現しました。 */}
          <div className="absolute inset-x-[-100px] inset-y-[-100px] z-0 bg-[radial-gradient(ellipse_at_center,rgba(56,189,248,0.1),transparent_70%)] pointer-events-none transform -rotate-[15deg]" />
          <div className="absolute inset-x-[-100px] inset-y-[-100px] z-0 bg-[radial-gradient(ellipse_at_center,rgba(56,189,248,0.08),transparent_70%)] pointer-events-none transform rotate-[25deg] delay-300" />

          {PAIN_CONTENT.measures.map((m, i) => (
            <div key={i} className={`relative grid lg:grid-cols-12 gap-0 items-center`}>
              
              {/* 背後の特大ID：デジタルな発光をやめ、背景に深く静かに溶け込む透かしデザイン */}
              ...
              `}>
                ...
                <div className="space-y-5 relative z-10">
                  <div className="flex items-center gap-3">
                    ...
                    {/* 💡 タグだけピカピカ光るビミョーな感じを廃止！
                       光を消して、背景の透過グレーに美しく馴染む上質なマットデザイン（大人の男らしさ）に変えました */}
                ...
                {/* 💡 【光源：インナーグロウ】各カードの内部に、ジュエルのように上品に美しく光る線の光源を追加。
                   これにより、透过させた背後のジグザグ光と重なり、さらに洗練された光の演出を表現しました。単純な線ではなく、中央から滲み出るようなボカシのある明るいモヤです。単純な線ではなく、中央から滲み出るようなボカシのある明るいモヤです。 */}
              ...
              {/* 💡 カードの下部を影みたいに当初のように下方向限定で光らせる（透過グレーに合わせて少し薄く） */}
              <div className="absolute inset-0 z-0 bottom-[-30px] left-1/2 -translate-x-1/2 w-[70%] h-[30px] bg-sky-400/8 blur-[20px] pointer-events-none rounded-full" />
            </div>
          ))}
        </div>
