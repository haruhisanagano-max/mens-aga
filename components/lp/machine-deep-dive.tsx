...
export default function MachineDeepDive() {
  // ---------------------------------------------------------
  // ★ デザイン統一用リモコン（AGA・アウターグロウ×強烈光源決定版）
  // ---------------------------------------------------------
  ...
  /* 💡 【修正①：上段スペックカード】（光源なしのアウターグロウ）
     影（Shadow）の設定を、単なるドロップシャドウから、医療ブルー（SkyBlue）の広範囲なぼかし（アウターグロウ）へ変更。
     光源の芯（白）は入れず、青いモヤだけがフレームから漏れ出るようにします。 */
  const mainCardBorder = "border border-slate-700/60 transition-all duration-500 hover:border-sky-400/20"
  const mainCardShadow = "shadow-[0_20px_50px_-10px_rgba(0,0,0,0.6),0_30px_90px_rgba(56,189,248,0.1)] bg-[#151F32]"

  /* 💡 【修正②：下部3連ロングカード】（少し明るい背景×強烈光源透過グレー）
     カード自体の存在感を出し、背後の光の透過感を上げるため、
     提供コードの成功例と同様の「明るいチャコールグレー（bg-[#1E293B]）」に透過度（/70）をかけて採用。 */
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <motion.div 
            ...
            style={gpuStyle}
          >
            {/* 上部カードの内部光：右上からふんわりと差し込む医療ブルーの光（光源なし） */}
          ...
          {/* 🔴 3. ロングカード（3連詳細・縦並び） */}
          <motion.div 
            /* 💡 カードの背景を「少し明るい透過グレー（bg-[#1E293B]/70）」へ（修正②）。
               これで背後の光の透過感が上がります。 */
            ...
            {MACHINE_CONTENT.reasons.map((reason, i) => (
              <div key={i} className={`relative grid md:grid-cols-2 items-center ${i !== 0 ? 'border-t border-slate-700支線 leading-loose font-medium pt-2 border-t border-slate-700/40' : ''}`}>
                ...
                <div className={`${cardTextPadding} relative h-full flex flex-col justify-center`}>
                  ...
                  {/* 💡 【各要素への扇状グロウ：独立発光】前回の修正（角度の違い）を維持 */}
                  {i === 0 && (
